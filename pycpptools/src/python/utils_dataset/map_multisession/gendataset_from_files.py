...
class DataGenerator:
	...
	def setup_directories(self):
		...
		for i in range(self.args.num_split):
			paths.append(os.path.join(base_path, f'seq{i}'))
		for path in paths:
	...
	def run(self):
		...
		path_segments = [
			(int(max(0, i * total_len / N - total_len / (3 * N))), int(min(total_len, (i + 1) * total_len / N + total_len / (3 * N))))
			for i in range(N)
		]
		print(f'Total length of segment: {total_len}, split {N} segments')
		print('Segments: ', path_segments)
		all_intrinsics, all_poses_abs = np.empty((0, 7), dtype=object), np.empty((0, 8), dtype=object)

		for seg_id, (start_ind, end_ind) in enumerate(path_segments):
			print(f'Processing segment {seg_id} from {start_ind} to {end_ind}')
			seg_poses_rel = np.empty((0, 8), dtype=object)
			...
			for ind in range(start_ind, end_ind):
				...
				vec[0, 0], vec[0, 1:5], vec[0, 5], vec[0, 6] = \
					f'seq{seg_id}/frame_{ind - start_ind:05d}.jpg', self.intrinsics[ind, :4], int(self.intrinsics[ind, 4]), int(self.intrinsics[ind, 5])
				...
				vec[0, 0], vec[0, 1:5], vec[0, 5:] = f'seq{seg_id}/frame_{ind - start_ind:05d}.jpg', np.roll(quat, 1), tsl
				...
				vec = np.empty((1, 8), dtype=object)
				vec[0, 0], vec[0, 1:5], vec[0, 5:] = f'seq{seg_id}/frame_{ind - start_ind:05d}.jpg', np.roll(quat_0t, 1), tsl_0t
				...
				new_rgb_img_path = os.path.join(self.base_path, f'seq{seg_id}', f'frame_{ind - start_ind:05d}.jpg')
				new_depth_img_path = os.path.join(self.base_path, f'seq{seg_id}', f'frame_{ind - start_ind:05d}.zed.png')			
				os.system(f'cp {rgb_img_path} {new_rgb_img_path}')
				os.system(f'cp {depth_img_path} {new_depth_img_path}')
			np.savetxt(os.path.join(self.base_path, f'poses_seq{seg_id}.txt'), seg_poses_rel, fmt='%s %.9f %.9f %.9f %.9f %.9f %.9f %9f')
		
		np.savetxt(os.path.join(self.base_path, 'intrinsics.txt'), all_intrinsics, fmt='%s %.9f %.9f %.9f %.9f %d %d')
		np.savetxt(os.path.join(self.base_path, 'poses_abs.txt'), all_poses_abs, fmt='%s %.9f %.9f %.9f %.9f %.9f %.9f %.9f')
